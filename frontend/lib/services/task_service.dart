...
class TaskService {
  ...
  Future<List<dynamic>> getTasks({String? search, String? status}) async {
    ...
    try {
      ...
      if (kDebugMode) print('Error en getTasks: $e');
      rethrow;
    }
  }
  ...
  Future<void> createTask({
    ...
    final token = await _getToken();
    final url = '${ApiConstants.baseUrl}/tasks';
    final data = {
      'title': title,
    ...
    if (kDebugMode) {
      print('--- AUDITORÍA CREATE TASK (ENVIANDO) ---');
      print('Data: $data');
    }
    ...
    try {
      ...
      if (kDebugMode) {
        print('--- AUDITORÍA CREATE TASK (RESPUESTA) ---');
